...
if ($_SESSION['stunum'] != null){
	echo "<script>location.href='./stumain.php';</script>";
    exit();
}
...
<body>
	<header style="width:100%; height:70px; /*background:rgba(0,181,173,0.8);*/ background:rgba(81,98,111,1); ">
		...
		<p style="font-size:20px; font-weight:600; margin:0 0 0 100px; float:left; color:#fff; line-height:70px;""/><span style="color:#A31432; color:#fff; font-size:30px">Reo_</span><span style="color:#A31432; color:#fff; font-size:20px">STUDENT</span>
					...
					<div class="ui stacked segment">
						<div class="field">
							<div class="ui left icon input">
								<i class="user icon"></i>
								<input style="color:rgba(81,98,111,1);" type="username" name="username" id="userid" placeholder="학번">
							</div>
							<div class="ui left icon input">
								<i class="lock icon"></i>
								<input style="color:rgba(81,98,111,1);" type="password" name="password" id="userpw" placeholder="비밀번호">
							</div>
						</div>
						<div id="login_btn" class="ui fluid large teal submit button" style="background:rgba(81,98,111,1);">Login</div>
						<div style="margin:10px 0 10px 0; width:262px; font-size:11px; color:rgba(81,98,111,1);  text-align:center;" id="login-result"></div>
					</div>
				</form>
				<!-- <p style="font-size:15px; padding-top:5px;  color:rgba(81,98,111,1);">Want to join??  <span style="color:rgba(81,98,111,1); cursor:pointer; text-decoration:underline;" id="join-view">  Click!</span></p>
			</div> -->
		</div>
	</article>
</article>
...
<!-- Script -->
	...
	function login(){
		...
		data.append("stunum", userid);
		data.append("stupw", userpw);
		$.ajax({
			type: "POST",
			url: "../db/stulogin.php",
			data: data,
